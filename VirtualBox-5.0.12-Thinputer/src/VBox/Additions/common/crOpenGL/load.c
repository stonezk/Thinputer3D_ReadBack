...
#include "cr_helper.h"
#include "cr_whiteList.h"
// ip:port tcpip://xxx.xxx.xxx.xxx:xxxxx
...
#endif
    //LogPrint(" error in  stuCheckWindowsState ");
...
{
    stubCheckWindowsState();
    origSwapBuffers(window, flags, 0, 0);
}
...
static void stubSPUSafeTearDown(void)
{
    ...
    crLockMutex(mutex);
#endif
    crDebug("##zr##: stubSPUSafeTearDown");
    LogPrint("start stubSPUSafeTearDown");
...
# if defined(WINDOWS)
    if (stub.hSyncThread && RTThreadGetState(stub.hSyncThread)!=RTTHREADSTATE_TERMINATED)
    {
        ...
        if (!hNative)
        {
		    ...
		    LogPrint("Failed to get handle for sync thread(%#x)", GetLastError());
        }
        else
        {
            crDebug("Got handle %p for thread %#x", hNative, RTThreadGetNative(stub.hSyncThread));
			LogPrint("Got handle %p for thread %#x", hNative, RTThreadGetNative(stub.hSyncThread));
        }
...
{
	crDebug("##zr##: stubExitHandler call stubSPUSafeTearDown ");
    stubSPUSafeTearDown();
...
{
	crDebug("##zr##: stubSignalHandler call stubSPUSafeTearDown %d", signo);
    stubSPUSafeTearDown();
...
#endif
	crDebug("##zr## Load.c: stubInitVars Create WinINfo");

	smem_win_init(&defaultWin->smem, 0);
    crHashtableAdd(stub.windowTable, 0, defaultWin);
	

#if 1
	crDebug("##zr## Load.c: stubInitVars call atexit");
...
stubInitLocked(void)
{
	...
	LogPrint("Stub launched for %s in stubInitLocked()", response);
    ...
    LogPrint("start crNetInit() in stubInitLoacked()");
    crNetInit( NULL, NULL );
...
#ifndef WINDOWS
    {
        ...
        ns.name = g_Host;
        ns.buffer_size = 1024;
		LogPrint("start crNetServerConnect() in stubInitLoacked()");
        ...
        if (!ns.conn)
        {
			...
			LogPrint("Failed to connect to host. Make sure 3D acceleration is enabled for this VM.");
            return false;
        }
        else
        {
            LogPrint("start crNetFreeConnection() in stubInitLocked()");
    ...
    for (i = 0 ; i < num_spus ; i++)
    {
		...
		LogPrint( "SPU %d/%d: (%d) \"%s\"", i+1, num_spus, spu_ids[i], spu_names[i] );
    }
...
#if defined(WINDOWS) && defined(VBOX_WITH_WDDM)
    hVBoxD3D = NULL;
    LogPrint("defined(WINDOWS) and defined(VBOX_WITH_WDDM)");
    if (!GetModuleHandleEx(0, VBOX_MODNAME_DISPD3D, &hVBoxD3D))
    {
        crDebug("GetModuleHandleEx failed err %d", GetLastError());
        hVBoxD3D = NULL;
    ...
    if (hVBoxD3D)
    {
		...
		LogPrint("running with %s", VBOX_MODNAME_DISPD3D);
        stub.trackWindowVisibleRgn = 0;
    ...
    LogPrint("start crSPULoadChain() in stubInitLocked()");
    ...
    LogPrint("Load pointers to native OpenGL functions into stub.nativeDispatch");
...
stubInit(void)
{
    ...
    if (!stub_initialized)
    {
    	crDebug("##zr## stubInit %d", stub_initialized);
    	bRc = stub_initialized = stubInitLocked();
		crDebug("##zr## stubInit ret %d, %d", stub_initialized, bRc);
    }
...
void detach_message(char *src, char *server_ip, char *terminal_ip, char *port)
{
    char *data = NULL;

    data = strtok(src, " ");
    if(data){
   	    crStrcpy(Terminal_ip, data);
		data = strtok(NULL, " ");
		crStrcpy(g_Host,"tcpip://");
		crStrcpy(g_Host+crStrlen("tcpip://"), data);
		data= strtok(NULL, " ");
		crStrcpy(tmp_port, data);
		//LogPrint("stone the terminal_ip:%s, server_ip:%s, port:%s", terminal_ip, server_ip, port);
   	}
}
...
BOOL WINAPI DllMain(HINSTANCE hDLLInst, DWORD fdwReason, LPVOID lpvReserved)
{
    ...
    case DLL_PROCESS_ATTACH:
    {
        ...
        HMODULE hCrUtil;
        char aName[MAX_PATH];
        //LogPrint("defined DEBUG_misha");
        GetModuleFileNameA(hDLLInst, aName, RT_ELEMENTS(aName));
        crDbgCmdSymLoadPrint(aName, hDLLInst);
        //LOG_DEBUG(aName);
        hCrUtil = GetModuleHandleA("ThinputerOGLcrutil.dll");
        Assert(hCrUtil);
        crDbgCmdSymLoadPrint("ThinputerOGLcrutil.dll", hCrUtil);
#endif
#ifdef CHROMIUM_THREADSAFE
        crInitTSD(&g_stubCurrentContextTSD);
#endif

		crDebug("##zr## load.c DllMain : DLL_PROCESS_ATTACH");
		smem_module_init();
...
# endif
...
#endif
		//crGetHostFromPipe(g_Host+crStrlen("tcpip://"))
		crGetHostFromPipe(Client_IP);
        detach_message(Client_IP, g_Host+crStrlen("tcpip://"), Terminal_ip, tmp_port);
		port_3D = atoi(tmp_port);
		SetIPAndPort( port_3D,Terminal_ip);

        crNetInit(NULL, NULL);
		LogPrint("stone terminal ip:%s and port:%d", Terminal_ip, port_3D);
        LogPrint("the g_host is:%s",g_Host);
		 //ns.name = "vboxhgcm://host:0";
        ns.name = g_Host;
        ns.buffer_size = 1024;
        ...
        if (!ns.conn)
        {
			...
			LogPrint("Failed to connect to host (is guest 3d acceleration enabled?), aborting ICD load.");
#ifdef VDBG_VEHANDLER
            if (g_VBoxVehEnable)
        ...
        {
             LogPrint("start crNetFreeConnection in DllMain()");
             crNetFreeConnection(ns.conn);
        }
...
*/
    ...
    case DLL_PROCESS_DETACH:
    {
        ...
        stubSetCurrentContext(NULL);
		//LogPrint("the type is DLL_PROCESS_DETACH");
        if (stub_initialized)
        {
            CRASSERT(stub.spu);
...
#endif
*/
		crDebug("##zr##: DLL_PROCESS_DETACH call stubSPUSafeTearDown ");
        stubSPUSafeTearDown();
		...
		smem_module_clean();
		crDebug("##zr## load.c DllMain : DLL_PROCESS_DETACH");
        break;
    }

    case DLL_THREAD_ATTACH:
    {
		crDebug("##zr## load.c DllMain : DLL_THREAD_ATTACH");
        if (stub_initialized)
        {
            CRASSERT(stub.spu);
    ...
    case DLL_THREAD_DETACH:
    {
		...
		crDebug("##zr## load.c DllMain : DLL_THREAD_DETACH");
        break;
    }

    default:
		LogPrint("##Not support message type in DllMain()##");
        break;
    }
