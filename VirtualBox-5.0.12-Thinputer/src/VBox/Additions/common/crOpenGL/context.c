...
# endif
		...
		crDebug("##zr## context.c: stubDestroyWindow Delte WinInfo %d", window);
		smem_win_clean(&winInfo->smem, (int)winInfo->hWnd);
...
stubNewWindow( const char *dpyName, GLint visBits )
{
	...
	crDebug("##zr## context.c:stubNewWindow Create WinInfo");
...
#endif
{
	...
	crDebug("##zr## context.c:stubGetWindowInfo  Create WinInfo hwnd %x---", hwnd);
...
#ifndef WINDOWS
	key = (unsigned int) drawable;
#else
	key = (unsigned int) hwnd;
#endif

	smem_win_init(&winInfo->smem, key);
	crHashtableAdd(stub.windowTable, key, winInfo);
...
#if 1
static void DrawSyncBitmap(HWND hwnd, int x, int y, int w, int h, const char * buff)
{
	HBITMAP hNewBMP, hOldBMP;
	HDC hDC, hMemDC;

	crDebug("###_#   CreateBitmap Success  %d,%d buffdata: %x,%x !!!", 
		w, h, *((int*)(buff)), *((int*)(buff+4))
		);
	//return;
	
	hNewBMP = CreateBitmap(w, h, 1, 32, buff);
	if ((int)hNewBMP == (-1))
	{
		crDebug("###_# CreateBitmap Error !");
		return;
	}
...
static void FillBlackColor(HWND hwnd)
{
	...
	HBRUSH rush;
	hdc = GetDC(hwnd);
	rush = CreateSolidBrush(RGB(0, 255, 0));
...
static void UseTransData(WindowInfo *window, struct smem_display *disp)
{
	const char *buff = (const char *)smem_display_get_data(disp);

	DrawSyncBitmap(window->hWnd, 0, 0, disp->width, disp->height, buff);
}



void stubSwapBuffers(WindowInfo *window, GLint flags)
{
    if (!window)
        return;

	crDebug("#_# context.c : stubSwapBuffers winType: %d ", (int)window->type);
...
/*      stub.wsInterface.CGLFlushDrawable( context->cglc ); */
		...
		if (window->smem.attach_key==(int)window->hWnd && 
				window->smem.mem!=NULL && window->smem.layout.display!=NULL) {

			stub.spu->dispatch_table.SwapBuffers( window->spuWindow, flags, 
				window->smem.layout.phy_display.low_addr, window->smem.layout.phy_display.high_addr);

			UseTransData(window, window->smem.layout.display);
			
		} else {
			crDebug("#_# : stubSwapBuffers Shared Memory Error!!!! ---- ");
		}
