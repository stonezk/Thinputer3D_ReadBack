...
)
{
	...
	LogPrint("start packspuConnectToServer()");
...
packspu_VBoxCreateContext( GLint con, const char *dpyName, GLint visual, GLint shareCtx )
{
	...
	crDebug("#zr# packspu_VBoxCreateContext Enter thread: %p, con %d", thread, con);

    CRASSERT(!curThread == !curPacker);
    CRASSERT(!curThread || !curPacker || curThread->packer == curPacker);
#ifdef CHROMIUM_THREADSAFE
	crDebug("#zr# packspu_VBoxCreateContext defined ( CHROMIUM_THREADSAFE )");
    crLockMutex(&_PackMutex);
#endif
    ...
    if (CRPACKSPU_IS_WDDM_CRHGSMI())
    {
    	crDebug("#zr# packspu_VBoxCreateContext Pos 1");
    ...
    {
    	crDebug("#zr# packspu_VBoxCreateContext Pos 2");
