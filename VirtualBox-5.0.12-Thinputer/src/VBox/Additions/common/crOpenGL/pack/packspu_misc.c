...
GLint PACKSPU_APIENTRY packspu_VBoxWindowCreate( GLint con, const char *dpyName, GLint visBits )
{
	...
	crDebug("##zr## packspu_misc.c: packspu_VBoxWindowCreate : %d", retVal);
...
GLenum PACKSPU_APIENTRY packspu_GetError( void )
{
    ...
    if (!CRPACKSPU_IS_WDDM_CRHGSMI() && !(pack_spu.thread[pack_spu.idxThreadInUse].netServer.conn->actual_network))
    {
        //LogPrint("packspu_GetError doesn't work when there's no actual network involved!\nTry using the simplequery SPU in your chain!");
...
GLuint PACKSPU_APIENTRY packspu_VBoxPackGetInjectID(GLint con)
{
    ...
    crLockMutex(&_PackMutex);
    {
        ...
        if (CRPACKSPU_IS_WDDM_CRHGSMI())
        {
			...
			crDebug("##_#: thread pos A is %p", thread);
        }
        else
        {
            CRASSERT(!con);
			...
			crDebug("##_#: thread pos B is %p", thread);
        }

		if (thread == NULL)
		{
			crDebug("##_# thread is NULL  error");
		}
		if (thread->netServer.conn == NULL)
		{
			crDebug("##_# thread->netServer.conn is NULL  error");
		}
		if (thread->netServer.conn->type!=CR_VBOXHGCM)
		{
			crDebug("##_# thread->netServer.conn->type is %d  error , except %d", 
				thread->netServer.conn->type, CR_VBOXHGCM);
		}
