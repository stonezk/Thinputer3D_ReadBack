#ifndef _RODAPP_LINUX_LIST_H
#define _RODAPP_LINUX_LIST_H
 ...
 * list_for_each_entry_rcu().
 */
static __inline void list_add_tail_rcu(struct list_head *new,
 ...
 */
static __inline void list_del_rcu(struct list_head *entry)
{
	__list_del(entry->prev, entry->next);
	entry->prev = LIST_POISON2;
}
 ...
 */
static __inline void list_replace_rcu(struct list_head *old,
				struct list_head *new)
{
	new->next = old->next;
	new->prev = old->prev;
 ...
 */
static __inline void list_splice_init_rcu(struct list_head *list,
					struct list_head *head,
					void (*sync)(void))
{
	struct list_head *first = list->next;
