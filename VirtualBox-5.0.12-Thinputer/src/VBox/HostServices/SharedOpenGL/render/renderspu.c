...
DoSync(void)
{
    ...
    if (render_spu.is_swap_master)
    {
		...
		LogPrint("stone Send data use TCPIP In DoSync()");
    }
    else
    {
...
WindowInfo* renderspuWinCreate(GLint visBits, GLint id)
{
	...
	crDebug("##zr## Renderspu.c: renderspuWinCreate Create WinINfo");
...
RENDER_APIENTRY renderspuWindowDestroy( GLint win )
{
    ...
    if (window) {
		...
		crDebug("##zr## Rnderspu.c: renderspuWindowDestroy Delte WinInfo");
    }
    else {
                                                            ...
                                                            const GLvoid *values)
{
    ...
    LogPrint("stone Into renderspuChromiumParametervCR()");
    switch (target) {
    ...
    case GL_GATHER_DRAWPIXELS_CR:
        ...
        LogPrint("stone the target type is GL_GATHER_DRAWPIXELS_CR");
        for (client_num=0; client_num< render_spu.server->numClients; client_num++)
        {
