                         ...
                         const void *start, unsigned int len )
{
    ...
    int send_len = len;
    LZ4_stream_t *stream_t;
    stream_t = LZ4_createStream();
    //LZ4_resetStream(stream_t);
    ...
    if (!bufp) {
...
#ifdef IN_GUEST
        size_t com_len = LZ4_compressBound(len);
        void * com_buf = calloc(1, com_len);
        memset(com_buf,0,com_len);
        LZ4_resetStream(stream_t);
        com_len=LZ4_compress_fast_continue(stream_t, start,com_buf,send_len,com_len,2);
        //the last para is accelerate ,the value bigger,rate slower,ratio larger
        ...
        LZ4_resetStream(stream_t);
        if (com_len=LZ4_compress_fast_continue(stream_t,start,com_buf+sizeof(CR_LENGTH_INFO),len,com_len,2))
        {
