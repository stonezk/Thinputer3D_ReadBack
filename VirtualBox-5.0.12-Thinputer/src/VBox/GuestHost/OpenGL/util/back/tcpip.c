...
static const LZ4F_preferences_t lz4_preferences = {	
       { LZ4F_max256KB, LZ4F_blockIndependent, LZ4F_noContentChecksum, LZ4F_frame, 0, { 0, 0 } },
...
{
    CR_LENGTH_INFO crSendLenInfo = {0};
    const void * send_buf = start;
    int send_len = len;
    ...
    if (!bufp) {
...
#ifdef IN_GUEST
        size_t com_len = LZ4F_compressFrameBound(len,&lz4_preferences);
        void * com_buf = calloc(1, com_len);
        memset(com_buf,0,com_len);
        com_len=LZ4F_compressFrame(com_buf, com_len, start, send_len,&lz4_preferences);
        LogPrint("压缩完成,压缩前长度为 %d 压缩后长度为 %d",com_len,send_len);
        send_buf = com_buf;
        send_len = com_len;
        crSendLenInfo.comLen = conn->swap ? SWAP32(com_len) : com_len;
        
#endif
        crSendLenInfo.orgLen = conn ->swap ? SWAP32(len) : len;
        LogPrint("原始内容为:%s",(char*)start);
        LogPrint("ComPress %s",(char*)com_buf);
        crTCPIPWriteExact( conn, &crSendLenInfo, sizeof(crSendLenInfo));
        if (!conn || conn->type == CR_NO_CONNECTION)
            return;
        crTCPIPWriteExact( conn, send_buf,send_len);
        //LogPrint("Send buffer!!!!");
#ifdef IN_GUEST
        if (com_buf)
            free(com_buf);
        com_buf = NULL;
        ...
        //unsigned int *lenp;
        CR_LENGTH_INFO *send_buf = NULL;
        int send_len = len + sizeof(CR_LENGTH_INFO);
       
#ifdef IN_GUEST
        char *com_buf;
        unsigned int com_len = LZ4F_compressFrameBound(len,&lz4_preferences);
#endif
         ...
         */
#ifdef IN_GUEST
        com_buf = (char *)calloc(1, com_len+sizeof(CR_LENGTH_INFO));
        memset(com_buf,0,com_len+sizeof(CR_LENGTH_INFO));
        if (com_len=LZ4F_compressFrame(com_buf+sizeof(CR_LENGTH_INFO),com_len, start,len,&lz4_preferences))
        {
         // LogPrint("压缩数据和长度一起发送 压缩前长度 %d 压缩后长度 %d",len,com_len);
            send_len = com_len + sizeof(CR_LENGTH_INFO);
            send_buf = (CR_LENGTH_INFO *)com_buf;
            send_buf->orgLen = conn->swap ? SWAP32(len) : len;
            send_buf->comLen = conn->swap ? SWAP32(com_len) : com_len;

            com_buf = NULL;
        }else
#endif
        {
            send_buf = (CR_LENGTH_INFO *)calloc(1, send_len);
            send_buf->orgLen = conn->swap ? SWAP32(len) : len;
            memcpy(send_buf+1, start, len);
        }
        //LogPrint("原始内容为:%s",(char*)start);
        //LogPrint("ComPress %s",com_buf);
        crTCPIPWriteExact(conn, send_buf, send_len);
...
crTCPIPReceiveMessage(CRConnection *conn)
{
    ...
    void *recv_buf;
    void *uncom_buf;
	CR_LENGTH_INFO crLengthInfo;
    LZ4F_errorCode_t errorCode;
    LZ4F_decompressionContext_t decContext;
    crLengthInfo.orgLen = 0;
    crLengthInfo.comLen = 0;
    ...
    if (crLengthInfo.comLen > 0)
    {
        ...
        errorCode = LZ4F_createDecompressionContext(&decContext,LZ4F_VERSION);  //LZ4F decompress
        if(LZ4F_isError(errorCode))
          return;
        len=LZ4F_decompress(decContext,uncom_buf,&(int)len,recv_buf,&(int)total,NULL);
