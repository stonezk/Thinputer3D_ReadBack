...
/**
 *crSubString split the str from start to end
 *
*/
char * crSubString(const char *str,int start,int len)
{
    char * subStr;
    int length = crStrlen(str);
    if(!str || start > length || len <=0)
        return NULL;
    if(len >= length)
    {
        subStr = str;
    ...
    if(start + len > length)
    {
        subStr = (char *)crAlloc((length - start+1)*sizeof(subStr));
        crMemcpy(subStr,str+start,(length-start));
        subStr[length-start]='\0';
        return subStr;
    }
        
    subStr=(char *)crAlloc((len+1)*sizeof(subStr));
    crMemcpy(subStr,str+start,len);
    subStr[len]='\0';
    return subStr;
}
/**
