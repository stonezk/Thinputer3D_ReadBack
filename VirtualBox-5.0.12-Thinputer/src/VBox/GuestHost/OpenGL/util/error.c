...
DECLEXPORT(void) crWarning(const char * strOutputString,...)
{
...
static int set_time_string(char *buff)
{
	SYSTEMTIME sys;
	int n;
	...
	n = sprintf(buff, " %d:%d %d-%d  ", sys.wHour, sys.wMinute, sys.wSecond, sys.wMilliseconds);

	return n;
}

DECLEXPORT(void) crDebug(const char * strOutputString,...)
{
    char strBuffer[4096]={0};
	int offset;
    va_list vlArgs;
	...
	offset = set_time_string(strBuffer);
	
    _vsnprintf(strBuffer+offset,sizeof(strBuffer)-1-offset,strOutputString,vlArgs);
    //vsprintf(strBuffer,strOutputString,vlArgs);
    va_end(vlArgs);
    OutputDebugString(strBuffer);
}
