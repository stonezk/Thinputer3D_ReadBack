...
//get time of send message
 static DWORD send_time;
        ...
        )
{
    ...
    {
        cr_net.use_tcpip++;
        LogPrint("using the tcpip protocol!!!!!!!");
...
)
{
    ...
    LogPrint("In crNetConnectToServer( \"%s\", port=%d, mtu=%d, broker=%d )",
                    server, default_port, mtu, broker);
    ...
    if ( !crParseURL( server, protocol, hostname, &port, default_port ) ) {
         //LogPrint("Malformed URL: \"%s\"", server);
    ...
    LogPrint( "Connecting to %s on port %d, with protocol %s",
            hostname, port, protocol );
    ...
    if (!crStrcmp(protocol, "sdp")) {
		...
		LogPrint("SDP rename hostname: %s", hostname);
    }
#endif
    ...
    if (!crNetConnect( conn ))
    {
        ...
        LogPrint("crNetConnectToServer() failed, freeing the connection");
        #ifdef CHROMIUM_THREADSAFE
	...
	LogPrint( "##Connecting to %s (swapping=%d)", server, conn->swap );
    return conn;
}
...
//add szk
void SetIPAndPort(int port, char *ip)
{
	port_3D = port;
	memcpy(Terminal_ip, ip, 16);
	return;
}

int Get_Port_3D()
{
	port_3D = max(port_3D, 7050);
	return port_3D;
}
...
/**
 *Send a message to the receiver that set the connection's PID
 *called only by client
**/
void crNetSetPid(CRConnection *conn,unsigned long long pid)
{
    unsigned int len = sizeof(CRMessageSetPid);
    CRMessageSetPid msg;

    //LogPrint("stone Not send pid!!!");
	//return;
    CRASSERT(conn);

    if(conn->swap)
    {
        msg.header.type =(CRMessageType) SWAP32(CR_MESSAGE_SET_PID);
        msg.pid = (unsigned long long)SWAP64(pid);
    }
    else
    {
        msg.header.type = CR_MESSAGE_SET_PID;
        msg.pid = pid;
    }

    memcpy(msg.terminal_ip, Terminal_ip, 16);

    crNetSend( conn, NULL, &msg, len);    
}


crNetHandleMatchValue(CRConnection *conn,CRMessageMatchValue *msg)
{
	LogPrint("##Set conn:(%d)'s server_socket value as:%d", conn->tcp_socket, msg->match_value);
	conn->server_socket = msg->match_value;
}
...
crNetSend(CRConnection *conn, void **bufp, const void *start, unsigned int len)
{
	...
	//LogPrint("stone conn %d Send data in crNetSend() and the data len %d",conn->tcp_socket, len);
    //send_time = GetTickCount();
    //LogPrint("Message send time is %lu \n",send_time);
}

void
crNetSend_VIO(CRConnection *conn, void **bufp, const void *start, unsigned int len)
{
    CRMessage *msg = (CRMessage *) start;
    ...
    msg->header.conn_id = conn->id;
	LogPrint("##Conn %d Send data in crNetSend_VIO() and the data len %d", conn->tcp_socket,len);
...
{
    port_3D = max(port_3D, 7050);
	ns->conn = crNetConnectToServer( ns->name, port_3D,
                                     ns->buffer_size, 0
#if defined(VBOX_WITH_CRHGSMI) && defined(IN_GUEST)
                , pHgsmi
#endif
...
#ifdef IN_GUEST
# pragma message("HERE_____________________________________________________________")
...
static void crNetRecvWriteback( CRMessageWriteback *wb )
{
    ...
    //LogPrint("In crNetRecvWriteback time is: %.0f s\n",difftime(wb_time , send_time));
}
...
crNetRecvReadback( CRMessageReadback *rb, unsigned int len )
{
    ...
    LogPrint("##Into crNetRecvReadback()##");
...
crNetRecvReadPixels( const CRMessageReadPixels *rp, unsigned int len )
{
   ...
   /* set dest_ptr value */
  // LogPrint("1@@@@@ dest_ptr=%x,rp=%x, rp->pixels=%d,%d",&dest_ptr, rp, &(rp->pixels),sizeof(dest_ptr));
   crMemcpy( &dest_ptr, &(rp->pixels), sizeof(dest_ptr));
  // LogPrint("2@@@@@ %x,%x,",dest_ptr,&(rp->pixels));
   ...
   if (rp->alignment == 1 &&
      ...
      //LogPrint("in crNetRecvReadPixels  width=%d,height=%d,dest_ptr=%x,format=%d,type=%d,src_ptr=%x \n",
                                        //rp->width,rp->height,dest_ptr,rp->format,rp->type,src_ptr);
...
crNetDefaultRecv( CRConnection *conn, CRMessage *msg, unsigned int len )
{
	...
	#ifdef DEBUG_MODE
    //LogPrint("##Into crNetDefaultRecv() and msg type is:");
    #endif
    switch (pRealMsg->header.type)
    {
        ...
        case CR_MESSAGE_OOB:
            ...
            break;
        case CR_MESSAGE_SET_PID:
            break;
		case CR_MESSAGE_MATCH_VALUE:
			LogPrint("##The Recv Message's Type is CR_MESSAGE_MATCH_VALUE##");
			crNetHandleMatchValue(conn, &(pRealMsg->matchvalue));
			return;
        /*case CR_MESSAGE_SAVE_PIXELS:
            LogPrint("recv the message type CR_MESSAGE_SAVE_PIXELS!!!");
            break;
        */
        case CR_MESSAGE_READ_PIXELS:
            LogPrint("Can't handle Msg Type:CR_MESSAGE_READ_PIXELS ");
            WARN(( "Can't handle read pixels" ));
            return;
        case CR_MESSAGE_WRITEBACK:
            //LogPrint("message is CR_MESSAGE_WRITEBACK!!");
#ifdef IN_GUEST
# pragma message("-----------------------------------")
        ...
        case CR_MESSAGE_READBACK:
            //LogPrint("recvmessage type is CR_MESSAGE_READBACK!!");
#ifdef IN_GUEST
...
{
    //LogPrint("Start dispatch msg=%d,len=%u \n",msg->header.type,len);
    for ( ; rfl ; rfl = rfl->next)
    {
        if (rfl->recv( conn, msg, len ))
        {
             ...
             * All done.
             */
            //LogPrint("Message dispatched to SPU %d %u \n",msg->header.type,len);
            return;
        }
    }
...
{
    //LogPrint("Message dispatched to Default Recv \n");
...
*/
    ...
    LogPrint("crNetHostCapsGet supportted by HGCM only!!!");
    return 0;
}
#endif
