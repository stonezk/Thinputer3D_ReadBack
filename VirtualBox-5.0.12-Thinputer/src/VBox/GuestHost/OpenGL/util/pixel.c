...

{
    ...
    if (srcType == GL_BITMAP)
    {
        ...
        for (i=0; i<height; i++) {
            //LogPrint("in crPixelCopy2D i = %d",i);
    ...
    {   //LogPrint("in crPixelCopy2D srcType != GL_BITMAP ");
        CRASSERT(dstType != GL_BITMAP);
        ...
        //LogPrint("dstPacking->skipRows = %u,dstRowStridBytes= %d",dstPacking->skipRows,dstRowStrideBytes);
        /* handle skip rows */
        src += srcPacking->skipRows * srcRowStrideBytes;
        dst += dstPacking->skipRows * dstRowStrideBytes;
        //LogPrint("dstPacking->skipPixels = %u,dstBytesPerPixel= %d",dstPacking->skipPixels,dstBytesPerPixel);
        /* handle skip pixels */
        ...
        if (srcFormat == dstFormat && srcType == dstType)
        {
            ...
            {
                //LogPrint("in crMemcpy dst=%x,src=%x,length=%d",dst,src,height * srcBytesPerRow);
                //LogPrint("src first=%c,src last=%c", *((char*)src),*((char*)(src + height * srcBytesPerRow -1)));
                //LogPrint("dst=%c,dst last =%c",*((char *)dst),*((char*)(dst+height*srcBytesPerRow -1)));
        ...
        else
        {
            ...
            for (i = 0; i < height; i++)
            {
                ...
                if (srcPacking->swapBytes) {
                    ...
                    //LogPrint("swapRow=%x,src=%x,length=%d",swapRow,src,bytes);
