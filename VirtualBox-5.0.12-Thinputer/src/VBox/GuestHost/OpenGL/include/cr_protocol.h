...
typedef enum {
    ...
    CR_MESSAGE_SET_PID,
    CR_MESSAGE_MATCH_VALUE
    //CR_MESSAGE_SAVE_PIXELS
} CRMessageType;
...
typedef struct CRMessageSetPid {
    CRMessageHeader        header;
	char terminal_ip[16];
    unsigned long long     pid;
} CRMessageSetPid;

typedef struct CRMessageMatchValue{
	CRMessageHeader        header;
	unsigned int 		   match_value;
}CRMessageMatchValue;

/*typedef struct CRMessageSavePixels {
    CRMessageHeader        header;
    int                    x, y;
    int                    width, height;
    int                    format;
...
typedef union {
    ...
    CRMessageSetPid      setpid;
	CRMessageMatchValue  matchvalue;
    //CRMessageSavePixels  savePixels;
} CRMessage;
