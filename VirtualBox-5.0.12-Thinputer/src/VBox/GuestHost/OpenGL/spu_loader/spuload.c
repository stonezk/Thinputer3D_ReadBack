...
#endif
#define DLL_PREFIX "ThinputerOGL"
...
#endif /* DARWIN */
#ifdef VBOX     
                LogPrint ( "path= %s,length of path = %d, Dll_prefix = %s,name = %sspu%s",path,sizeof(path),DLL_PREFIX,name,DLL_SUFFIX);
                snprintf ( path, sizeof(path), "%s%sspu%s", DLL_PREFIX, name, DLL_SUFFIX );
#else
                sprintf ( path, "%s%sspu%s", DLL_PREFIX, name, DLL_SUFFIX );
                LogPrint ( "path= %s,Dll_prefix = %s,name = %sspu%s",path,DLL_PREFIX,name,DLL_SUFFIX);
...
SPU * crSPULoad( SPU *child, int id, char *name, char *dir, void *server )
{
        ...
        if (!the_spu->entry_point)
        {
				...
				LogPrint( "Couldn't load the SPU entry point \"%s\" from SPU \"%s\"!",
                                SPU_ENTRY_POINT_NAME, name );
        ...
        if (crStrcmp(the_spu->name,"error"))
        {
				...
				LogPrint("the spu name is error");
        ...
        {
                /* the default super/base class for an SPU is the error SPU */
				LogPrint("the spu name is hosterror");
                if (the_spu->super_name == NULL)
                {
		...
		LogPrint("Initializing %s SPU", name);
        the_spu->function_table = the_spu->init( id, child, the_spu, 0, 1 );
        LogPrint("the function list %s",the_spu->function_table->table[0].name);
        ...
        for (i = count-1 ; i >= 0 ; i--)
        {
                ...
                if (child_spu != NULL)
                {
                         ...
                         * still get updated when API's change on the fly. */
                         LogPrint("the child_spu isnot NULL");
